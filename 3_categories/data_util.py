...
def dl_filedata(filename, uncorrupted, return_infos=False):
    ...
    if return_infos:
        ...
        infos[:, 0] = data_files
        infos[:, 1] = np.array([str(fib_id) for valid_ids in all_valid_ids
                                for fib_id in valid_ids])
    ...
    for index, (pm, valid_ids) in enumerate(zip(pmu, all_valid_ids)):
        ...
        targets_int = np.array([0 if element[0] == 'QSO'
    ...
    if uncorrupted:
        fig = plt.hist(Y, bins=np.arange(4), align='left')
        plt.xticks(np.arange(3),
                   labels=['QUASAR', 'GALAXY', 'OTHER'],
...
def generate_train_test_datasets(ratio=0.7):
    ...
    indexes_train_interm = [category_indexes[i][:minimum[i]]
                            for i in collection]
    ...
    indexes_test_interm = [category_indexes[i][minimum[i]:]
                           for i in collection]
